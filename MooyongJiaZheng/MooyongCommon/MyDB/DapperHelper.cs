...
namespace MooyongCommon.MyDB
{
    ...
    public class DapperHelper : IDapperHelper, IDisposable
    {
        ...
        public DapperHelper(string conn = "", string jsonConfigFileName = "appsettings.json", DatabaseType databaseType = DatabaseType.MySql)
        {
            ...
            {
                conn = config.GetSection("ConnectionStrings:DefaultConnection").Value;
            }
        ...
        {
            return Connection.Connection.QueryFirst<T>(sql, param, transaction, commandTimeout, commandType);
        }
